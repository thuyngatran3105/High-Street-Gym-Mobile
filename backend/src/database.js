...
export function convertToMySQLDate(date) {
  ...
  //   const year = datetime.toLocaleString('default', {year: 'numeric', month: '2-digit'});
  ...
  // export function convertToMySQLDatetime(datetime) {
  //   // Convert to local date and time in the desired format
  //   const options = { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', hour12: true };
  //   const localDateTime = datetime.toLocaleString('en-US', options);
    
  //   // Format into MySQL datetime format (YYYY-MM-DD HH:mm AM/PM)
  //   const [month, day, year] = localDateTime.split(', ')[0].split('/');
  //   const [time, ampm] = localDateTime.split(', ')[1].split(' ');
    
  //   return `${year}-${month}-${day} ${time}${ampm.toUpperCase()}`;
  // }
